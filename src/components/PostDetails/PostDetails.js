  ...
  if (isLoading) {
    return (
      <Paper elevation={6} className={classes.loadingPaper}>
        <CircularProgress size="7em" color="error"/>
      </Paper>
    );
  }
    ...
    <Paper style={{ padding: '20px', borderRadius: '15px' }} elevation={6}>
      <div className={classes.card}>
        ...
        <div className={classes.imageSection}>
          <img className={classes.media} src={post.selectedFile || defaultimag} alt={post.title} />
        </div>
      </div>
          ...
          <div className={classes.recommendedPosts}>
            {recommendedPosts.map(({ title, name, message, likeCount, selectedFile, _id }) => (
              <div style={{ margin: '20px', cursor: 'pointer' }} onClick={() => openPost(_id)} key={_id}>
                ...
                <img src={selectedFile || defaultimag} width="200px" alt=''/>
              </div>
            ))}
          </div>
        </div>
